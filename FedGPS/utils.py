...
def proto_aggregation(clients_noise_protos: dict, clients_proto_agg_weights: dict) -> dict:
   
    agg_protos_label = {}
    agg_weights_label = {}
    
    for client_idx, client_protos in clients_noise_protos.items():
        for label, proto in client_protos.items():
            if label not in agg_protos_label:
                agg_protos_label[label] = []
                agg_weights_label[label] = []
            agg_protos_label[label].append(proto)
        
            weight = clients_proto_agg_weights[client_idx].get(label, 0)
    ...
    for label, proto_list in agg_protos_label.items():
        if len(proto_list) > 1:
            ...
            proto_stack = torch.stack(proto_list)  # shape: [num_clients, feature_dim]
            weights = torch.tensor(agg_weights_label[label], 
                                 device=proto_stack.device)  # shape: [num_clients]
            
           
            weights = weights / weights.sum()
            
            weighted_proto = (weights.unsqueeze(1) * proto_stack).sum(dim=0)
            weighted_agg_protos[label] = weighted_proto
        else:
            weighted_agg_protos[label] = proto_list[0]
    
    return weighted_agg_protos
